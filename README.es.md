...
Referencia técnica en español del proyecto académico. La configuración local requerida se explica en el [README principal](README.md#run-the-local-stack).
...
| **Framework HTTP** | Express (API 4; búsqueda 5) |
...
Antes del despliegue, preparar los manifiestos locales de secrets y del realm de Keycloak. Estos archivos no están versionados y las plantillas mencionadas por la guía anterior no aparecen en el árbol público. El generador de datos requiere una clave Gemini y que `DB_ENGINE` coincida con el motor desplegado.
...
- **Kubernetes:** ejecutar `kubectl port-forward -n proyecto01-restaurante svc/api-service 3000:80` y abrir `http://localhost:3000/api-docs`.
...
> Los reportes se generan localmente en `apps/api/coverage/` al ejecutar las pruebas; no están publicados en este repositorio.
>
> Las pruebas HTTP simulan dependencias externas. El umbral es una condición de Jest/CI, no una cifra nueva de cobertura medida.
